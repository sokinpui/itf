...
class ProgressBar:
    ...
    def __init__(self, total, prefix='Updating buffers:', length=40):
        self.total = total
        self.prefix = prefix
        self.length = length
        self.current = 0
    ...
    def update(self, step=1):
        ...
        # MODIFIED: Added item count for better context
        progress_str = f"[{self.current}/{self.total}]"
        sys.stdout.write(f'\r{self.prefix} |{bar}| {progress_str} {percent:.1f}%')
        sys.stdout.flush()
